...
Doorkeeper.configure do
  ...
  #  # end
   ...
   resource_owner_from_credentials do |_routes|
       user = User.find_by(email: params[:username] || params.try(:user).try(:email))
       user&.valid_password?(params[:password] || params.try(:user).try(:password)) ? user : nil
   end
  ...
  resource_owner_authenticator do
      if warden.user(scope: :user)
          ...
          throw(:warden, scope: :user)  # Sends user to Devise login page
      end
  end
  ...
  # you can allow to fall back to another strategy. For user upgrading
